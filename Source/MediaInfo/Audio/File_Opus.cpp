...
static const string Opus_Config(int8u config)
{
    ...
    for (const auto& config_it : config_table) {
        if (config >= config_it.start && config <= config_it.end) {
            int index = config - config_it.start;
            if (index >= 0 && index < config_it.frame_sizes_ms.size())
...
void File_Opus::Identification()
{
    ...
    if (FromIamf && ch_map) {
        ...
        Trusted_IsNot("Opus ID Header does not conform with ChannelMappingFamily = 0");
        return;
    }
    if (ch_map)
    {
